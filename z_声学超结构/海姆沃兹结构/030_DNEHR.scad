...
// 计算第 r 行之前的累计高度（按 H 累加）
function prev_rows_height(r) =
    sum([for (rr = [0:r-1]) params[rr*cols][6]]);

// 计算某一行中，第 c 列之前的累计宽度（按 W 累加）
function prev_cols_width(r, c) =
    sum([for (cc = [0:c-1]) params[r*cols + cc][4]]);

// 整体大块尺寸
total_w = row_total_width(0);   // 45.8
total_h = sum([for (r = [0:2]) params[r*cols][6]]);   // 45.6

// 把整体放到以原点为中心的位置
for (r = [0:2]) {
    row_h = params[r*cols][6];   // 每一行的 H 相同
