...
// 参数设置（现在全表示内部尺寸）
bottom_length = 99;    // 底部内边长
bottom_width  = 50.5;     // 底部内边宽
top_length    = 10;     // 顶部内边长
top_width     = 5;      // 顶部内边宽
height        = 50;     // 总高度
...
# translate([0, 0, -20])
    difference(){
        cuboid([bottom_length + 2*wall_thickness, bottom_width + 2*wall_thickness, height * 1.5], anchor = [0, 0, -1]);
        cuboid([bottom_length , bottom_width, height * 1.5 - wall_thickness], anchor = [0, 0, -1]);
        translate([0, 15, 0])
            cuboid([bottom_length , bottom_width, 20], anchor = [0, 0, -1]);
    }
